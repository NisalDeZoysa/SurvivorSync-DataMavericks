...
const VolunteerAssignment = () => {
  ...
  const fetchVolunteers = async () => {
    try {
      const response = await fetch(
        "http://localhost:7000/api/users/volunteers"
      );
  ...
  const fetchDisasterRequests = async () => {
    try {
      ...
      console.log("Disaster Requests:", data);
    } catch (error) {
  ...
  const fetchAssignments = async () => {
    try {
      const response = await fetch(
        "http://localhost:7000/api/assignments/summary"
      );
      const data = await response.json();
      setAssignments(data);
    } catch (error) {
  ...
  const handleAddAssignment = async () => {
    try {
      console.log("Adding assignment with data:", formData);
  ...
  const handleEditAssignment = async () => {
    try {
      const response = await fetch(
        `http://localhost:7000/api/assignments/${selectedAssignment.id}/status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            Assign and manage volunteer tasks across districts
          </p>
        </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      DisasterRequestId: e.target.value,
                    })
                  }
                  className="w-full p-2 border border-gray-300 rounded-md"
                >
                  ...
                  {disasterRequests.map((request) => (
                    <option key={request.id} value={request.id}>
                      {(request.district || "Unknown").split(" ")[0]}{" "}
                    ...
                    <TableRow key={assignment.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            {(assignment.District || "Unknown").split(" ")[0]},
                            {(assignment.Province || "Unknown").split(" ")[0]}
                          </span>{" "}
                          <a
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              >
                ...
                {disasterRequests.map((request) => (
                  <option key={request.id} value={request.id}>
                    {(request.district || 'Unknown').split(" ")[0]} {request.id} -{" "}
                    {request.name}
                  </option>
                ))}
              </select>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-gray-700">
                  {selectedAssignment.VolunteerName}
                </p>
              </div>
              <div>
                <Label className="font-semibold">District</Label>
                <p className="text-gray-700">{selectedAssignment.District || 'Unknown'}</p>
              </div>
              <div>

...
# class VerifiedResponseFormat(BaseModel):  # Fixed class name
#     """Respond to the user in this format."""
#     help_needed_requests: list[int] = []
#     status : Literal['pending', 'completed','error'] = 'pending'
#     verification_status: Literal['pending', 'verified', 'not_verified'] = 'pending'  # Fixed field name
#     verification_message: str
...
#         'Your task is to verify the disaster request information provided by the user and respond in the following JSON format.'
#         'Use the available tools to verify the information.'
#         'Get all the ids from the data rows return from the request_count tool. and add them to the help_needed_requests list.'
#         'If the tool calling gives sql query data rows, set the status to "completed"'
#         'If the return query count from request_count tool is greater than 2, set the verification_status to "verified".'
#         'If the return query count from request_count tool is greater than 5, set the verification_status to "already_verified".' 
#         'If the return query count from request_count tool is less than 2, set the verification_status to "not_verified".'
...
#         '''
#         {
#             "help_needed_requests": [<list of int>],
#             "status": "pending" | "completed" | "error",
#             "verification_status": "pending" | "verified" | "already_verified" | "not_verified",
...
#             try:
#                 # Extract JSON from the AI message
...
#                 return {
#                     'is_task_complete': False,
#                     'content': 'Invalid JSON response from agent'
#                 }
#         return {
#             'is_task_complete': False,
#             'content': 'No valid response from agent'
#         }
...
#             instructions = " ".join([part.get("text", "") for part in parts])
#         else:
#             instructions = str(message_data)
        
#         print(f"Request Verify Agent received task {task_id} with text: '{instructions}'")
#     except Exception as e:
#         print(f"Error extracting user text for task {task_id}: {e}")
...
#         print("Verify Agent Respond" + agent_response.json())
#         agent_response_format = agent_response['content']
#         agent_response_format_dict = agent_response_format.model_dump()
        
#         # Handle error responses
#         if isinstance(agent_response, tuple) and agent_response[1] >= 400:
#             return jsonify(agent_response[0]), agent_response[1]
        
#         print(f"Request Verify Agent response for task {task_id}: {agent_response}")
        
#         # Create response payload
#         response_payload = {
#             "request_varifiY_agent":{
#                 "id": task_id,
#                 "status": "request-verify-agent-completed",
#                 "agent" : "request-verify-agent", 
...
#                 # "request-verify-agent-response": agent_response['content'],
#                 "request-verify-agent-response": [
#                     {
#                         "role":"request-verify-agent",
#                         "response": agent_response_format_dict
#                     }
#                 ]
#             }
#         }
        
#         print(f"Forwarding task {task_id} response is {RESOURCE_TRACKING_AGENT}")

#         # Forward to next agent
#         target_url = f"{RESOURCE_TRACKING_AGENT}/tasks/send"
#         # try:
#         #     response = requests.post(target_url, json=response_payload, timeout=60)
#         #     # response.raise_for_status()
...
#         #     return jsonify({
#         #         "id": task_id,
#         #         "error": f"Failed to forward to next agent: {str(e)}"
#         #     }), 502
...
#     return jsonify(response_payload.json())

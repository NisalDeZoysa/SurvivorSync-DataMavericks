...
from agentapi.ai_workflow.db import change_status_after_assign_resources, resource_fetch, update_request_status, requests_fetch,assign_resources
...
def run_agent_workflow(input_data: Dict[str, Any]):
...
@app.post("/ai_workflow", response_model=WorkflowResponse)
async def call_ai_workflow(request: WorkflowRequest):
    try:
        # Run your AI agent workflow
        final_state = run_agent_workflow(request.input)
        
        # Convert the Pydantic AgentState to dict for JSON response
